...
function initLogo(){
	$.ajax({
		...
		type:"post",
		data:{"userid":localStorage.accountid},
		dataType:"POST",
		success:function(data){
			if(data.flag){
				var res=data.result;
				var logophotos=(res.logo==undefined?"":res.logo);
				var lconname=(res.title==undefined?"":res.title);
...
function queryLoginCheck(){
	...
	var obj={"userid":name,"userpass":pwd,"loginid":name};
	if(name.length==0){
	...
	$.ajax({
		...
		success:function(data){
			if(data.flag){
				...
				sessionStorage.sessionID=userid;
				layer.msg(data.reason, {time: 1000}, function(){
					window.location.replace("home.html?"+window.btoa(window.encodeURIComponent("username="+username+"&userid="+userid)));
				})
			}else{
				layer.close(index);
