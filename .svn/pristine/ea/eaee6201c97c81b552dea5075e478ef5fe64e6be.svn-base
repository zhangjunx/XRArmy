		 ...
		 var $ = layui.$,
		  ...
		  table.render({
			    ...
			    cols:  [[
				      ...
				      {field:'holdername', title:'人员名称', width:250,sort:true},
...
//					limits:[15,20,30,40,50,60,70,80,90],
			 ...
			 form.on('submit(component-form-demo1)', function(data){
				 ...
				 for(var i=0;i<checkArr.data.length;i++){
					 ...
					 var groupid=checkArr.data[i].groupid;
					 saveInfo(groupid,holderno,holdername,photostr,i,num,index);
				 }
			 })
			 //点击取消
...
//提交数据
function saveInfo(groupid,holderno,holdername,photostr,i,num,index){
	var obj={"groupid":groupid,"holderno":holderno,"holdername":holdername,
			"state":getUrlParam("state"),"photostr":photostr,"loginid":window.top.loginid};
	$.ajax({
		url:url+'/IssueRecord/add',
		type:"POST",
		data:obj,
