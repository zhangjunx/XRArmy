...
@Api(tags="DeviceUnit",description="接口标题：设备层接口")
public class DeviceUnitController {
	@Autowired
	private IDeviceUnitService ics;
	@Autowired
	private IDeviceChannelService idcs;
	...
	public Map<String,Object> getList(DeviceUnit record,String name,Integer curpage,Integer pagesize){
		...
		try{
			...
			for(int i=0;i<list.size();i++){
				...
				String deviceno=String.valueOf(m1.get("deviceno"));
				m.clear();
				m.put("deviceno", deviceno);
				List<Map<String,Object>> exist=idcs.getChannelListService(m);
				if(exist!=null && exist.size()>0){
					m1.put("arr", exist);
				}
				data.add(m1);
