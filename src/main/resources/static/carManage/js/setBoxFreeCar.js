		  ...
		  table.on('tool(LAY-user-manage)', function(obj){
				   ...
				   if(obj.event === 'del'){
			  			  layer.confirm('确定取消?', function(index){
			  				$.ajax({
			  					url:carUrl+'car/carManager/synCancleCarInfo',
			  					type:"POST",
			  			  ...
			  			  layer.confirm('确定同步?', function(index){
			  				  ...
			  				  if(data.synStatus == "更新"){
				  				...
				  				$.ajax({
				  					url:carUrl+'car/carManager/synEditCarInfo',
				  					type:"POST",
	 ...
	 if(index == 0){//免派车辆
		...
		var ip2=$("#familyCar")[0].contentWindow.$("#passList li.curr").attr("data-ip");
		 layer.open({
			 type:2,
			 title:"家属车辆",
			 content:"familyCarList.html?ip="+ip2+"&id="+id2,
			 area:["90%","91%"],
		})
	 }
})
...
function getSynCarList(id,synStatus){
	$.ajax({
		url:carUrl+"car/carManager/getSynCarList",
		type:"POST",
