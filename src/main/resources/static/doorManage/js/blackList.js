		 ...
		 var $ = layui.$,
		  ...
		  table.render({
			    ...
			    cols:  [[
					  ...
					  {field:'holderno', title:'人员编号', width:250,templet:function(d){
						  return "B_"+d.id;
					  }},
				      {field:'holdername', title:'人员名称', width:250,sort:true},
					  {field:'idcode', title:'证件号码', width:250,sort:true},
...
//					limits:[15,20,30,40,50,60,70,80,90],
			 ...
			 form.on('submit(component-form-demo1)', function(data){
				 ...
				 for(var i=0;i<checkArr.data.length;i++){
					 var holderno="B_"+checkArr.data[i].id;
...
function saveInfo(boxid,boxip,holderno,holdername,photostr,i,num,index){
	var obj={"boxid":boxid,"boxip":boxip,"holderno":holderno,"holdername":holdername,
			"state":"B","photostr":photostr,"loginid":window.top.loginid};
	$.ajax({
		...
		success:function(res){
			...
			if((i+1) == num){//最后一次循环
				...
				layer.msg(res.reason,{time:1000},function(){
					initTable();
					parent.$("#synblack")[0].contentWindow.getSynCarList(getUrlParam("id"));
