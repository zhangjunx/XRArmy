	...
	layui.use(["form","table","laydate"],function(){
		 ...
		 table.render({
			    ...
			    cols:  [[
					  ...
					  //{field:'unitname', title:'单位名称', width:200,sort:true},
		  ...
		  table.on('tool(LAY-user-manage)', function(obj){
				   ...
				   if(obj.event === 'del'){
			  			  layer.confirm('确定取消?', function(index){
			  				  var obj1={"id":data.id,"state":"1","boxip":data.boxip,"boxid":data.boxid,
			  						  "holderno":data.holderno,"type":"dd","loginid":window.top.loginid}
			  				$.ajax({
			  			  ...
			  			  layer.confirm('确定同步?', function(index){
			  				  ...
			  				  var obj1={"id":data.id,"state":"1","boxid":data.boxid,"boxip":data.boxip,
...
function getSynCarList(id,synstatus){
	$.ajax({
		...
		type:"POST",
		data:{"boxid":id,"state":"1","synstatus":synstatus,"loginid":window.top.loginid},
		success:function(res){
			console.log(res);
			if(res.flag){
